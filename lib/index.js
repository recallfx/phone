...
module.exports = function (phone, country) {
	...
	if (country) {
		if (!phone) {
			return {
				...
				countryCode: iso3166.country_code,
				phone: null,
			};
		}
    ...
    // set new country code if it does not match first 3 numbers of a phone
    if (phone.indexOf(iso3166.country_code) === 0) {
      return {
        fullPhone: '+' + phone,
        country: iso3166.alpha3,
        countryCode: iso3166.country_code,
        phone: phone.substr(iso3166.country_code.length),
      };
    }

		return {
			fullPhone: '+' + iso3166.country_code + phone,
			country: iso3166.alpha3,
			countryCode: iso3166.country_code,
			phone: phone,
		};
